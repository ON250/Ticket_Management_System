...
if($_SESSION['category_id']!=1) {echo '<script>document.location="dashboard.php";</script>';}
$adminID=$_SESSION['nn'];
...
if(isset($_POST['assignTicket'])){
  ...
  if($reg) { 
      ...
      $inmail=mysqli_query($connect,"INSERT into mailBox(requestID,ticketID,sid,category,message,type,c_date) values('$request_id','$ticket_no','$id','$category','$Description','$type',NOW())");
          if($inmail) {echo '<script>alert("The Ticket '.$ticket_no.' has been Assigned to '.$subAdmin.' Successfully! ")</script>'; echo '<script>document.location="assignTickets.php";</script>';}
  } 
          ...
          <div class="box box-info">
            ...
            <div class="box-body">
              ...
              <?php } ?>
                ...
                <?php 
                $slc=mysqli_query($connect,"SELECT * FROM request where request.status=1 or request.status=2");
                while($get=mysqli_fetch_array($slc)){
                  $iddd=$get['sentBy'];
                  if($get['sentCategory']==1){
                    $kk=mysqli_query($connect,"SELECT * FROM admin inner join location on location.locationID=admin.location where adminID='$iddd' ");
                    $gtt=mysqli_fetch_array($kk);
                    $llm=$gtt['admin_fname'].' '.$gtt['admin_lname'];
                    $title='Admin';
                    $location=$gtt['location_name'];
                  }
                  else if($get['sentCategory']==2){
                    $kk=mysqli_query($connect,"SELECT * FROM subadmin inner join location on location.locationID=subadmin.location where subAdminID='$iddd'");
                    $gtt=mysqli_fetch_array($kk);
                    $llm=$gtt['subAdmin_fname'].' '.$gtt['subAdmin_lname'];
                    $location=$gtt['location_name'];
                    $title='Sub-Admin';

                  }
                  else if($get['sentCategory']==0){
                    $kk=mysqli_query($connect,"SELECT * FROM end_users inner join location on location.locationID=end_users.user_branch where userID='$iddd'");
                    $gtt=mysqli_fetch_array($kk);
                    $llm=$gtt['user_fname'].' '.$gtt['user_lname'];
                    $location=$gtt['location_name'];
                    $title='User';
                  }
                ?>
 <option value="<?php echo $get['requestID'];?>"><?php echo $get['ticket_no'];?> <?php echo $get['request_subject'] . ' ( From '.$title.' :  ';?> <?php echo $llm.')  ( <em> '.$location.'</em> )' ;?> </option>
              <?php } ?>
                  </select>
                </div>
                <div>
                  <textarea class="textarea" required="" name="Description" placeholder="Assign with Description..."

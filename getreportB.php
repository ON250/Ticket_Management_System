...
if($status==0)
{
	$output='
	 <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" border="1" id="example">
                       ...
                       if($depart==""){
                                                      $title='
                                                         <tr>
                                                          ...
                                                          <tr>
                                                             <th colspan="7"><center><B>Branch Location : '.$location_name; if($depart!="") { $title.=' and Department of '. $department;} $title.=' </B></center></th>
                                                            </tr>
                                                              <tr>
                                                          ...
                                                          else 
                     {
                                                      $title='
                                                         <tr>
                                                          ...
                                                          <tr>
                                                             <th colspan="6"><center><B>Branch Location : '.$location_name; if($depart!="") { $title.=' and Department of '. $department;} $title.=' </B></center></th>
                                                              <tr>
                                ...
                                $output.='
                                    <tr>
                                      ...
                                      if($depart=="") $output.='<th>Department</th>';
                                    $output.='
                                      <th>End-Users</th>
                                      <th>Total Tickets</th>
                                      <th>Open</th>
          ...
          $counter=1;
          //Dates conditionss
    if($depart!="") $sqliD=mysqli_query($connect,"SELECT *  FROM  end_users where user_department='$depart' and user_branch='$location' ");
    else $sqliD=mysqli_query($connect,"SELECT *  FROM  end_users inner join department on department.departID=end_users.user_department where user_branch='$location' ");
...
$sqli2=mysqli_query($connect,"SELECT *,COUNT(*) AS totTicket2  FROM request inner join end_users on request.sentBy=end_users.userID where sentBy='$userID' and request.status=2 and '$date1' <= sentOn ");
                  ...
                  $res4=mysqli_fetch_array($sqli4);
                        ...
                        $output.='
                                    <tr class="gradeU">
                                      <td>'.$counter.'</td>';
                              if($depart=="")    $output.='<td>'.$resD['depart_name'].'</td>';
                                  $output.='
